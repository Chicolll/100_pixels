...
var essayMarks=[];$('.essay').each(function(){essayMarks.push($(this).offset().left-200)});
var planetMarks=[];$('.object').each(function(){planetMarks.push($(this).offset().left-200)});
...
// var destinations=$.makeArray(essayMarks).concat($.makeArray(planetMarks));destinations.sort(function(a,b){return a-b});var destinationNext=destinations[0];$('ul.nav a.nextjump').bind('click',function(event){cancelLightMsg()
// stopSpeeding();var currentDist=(window.pageXOffset);
var destinations=$.makeArray(essayMarks).concat($.makeArray(planetMarks));destinations.sort(function(a,b){return a-b});
var destinationNext=destinations[0];$('ul.nav2 a.nextjump').bind('click',function(event){cancelLightMsg()
stopSpeeding();var currentDist=(window.pageXOffset);


console.log("pressed next");

function getCurrLineNext(){
  var destNum = 0;
  $.each(destinations,function(index,value){if(currentDist>=value-100){destinationNext=destinations[index+1]; destNum += 1} else{return false}});
  console.log(destNum);
  return destNum;
}
function getSpeedNext(){
  var currLine = getCurrLineNext();
...
$.each(destinations,function(index,value){if(currentDist>=value-100){destinationNext=destinations[index+1];} else{return false}});

$('html, body').stop().animate({scrollLeft:destinationNext},getSpeedNext(),'easeInOutQuad').promise().done(function () {showButton(); travelling = false;});event.preventDefault();

})



$('ul.nav2 a.prevjump').bind('click',function(event){cancelLightMsg()
stopSpeeding();var currentDist=(window.pageXOffset);

function getCurrLinePrev(){
  var destNum = 0;
  $.each(destinations,function(index,value){if(currentDist>=value-100){destinationNext=destinations[index+1]; destNum += 1} else{return false}});
  destNum -= 1;
  console.log(destNum);
...
function getSpeedPrev(){
  var currLine = getCurrLinePrev();
  if (currLine == 0) {
    return 5000;
  }
  else if (currLine == 1){
...
$.each(destinations,function(index,value){if(currentDist<=value+100){destinationNext=destinations[index-1]; return false}});

$('html, body').stop().animate({scrollLeft:destinationNext},getSpeedPrev(),'easeInOutQuad').promise().done(function () {showButton();})   ;event.preventDefault();})
